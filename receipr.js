...
// console.log(sear);
 function category(arr) {
    ...
    //filtered value in the Allitems
...
fetch("https://api.spoonacular.com/recipes/" + oneitem + "/information?apiKey=b2e81cdc50d04625aea3e403d8829fe0")
    ...
    .then(json => {
        ...
        for (let i = 0; i < arr2.length; i++) {
            ...
            // console.log(arr2[i].vegetarian);
            let vegOrNot = arr2[i].vegetarian == false ? "No" : "yes"
            li.innerText = "Name:" + arr2[i].title
            li1.innerText = "Vegetarian:" + vegOrNot
  ...
  let viewMore = document.querySelector(".views")
    viewMore.addEventListener("click",()=>{
        ...
        fetch("https://api.spoonacular.com/recipes/random?apiKey=994be35b9f1649a9b9eb6d4ac33874f7&number=10")
        .then(res => res.json())
        .then(json => { 
            let values = json.recipes;
            for (let i = 0; i < values.length; i++) {
